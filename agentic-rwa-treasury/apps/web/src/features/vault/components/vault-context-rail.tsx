...
export function VaultContextRail({ snapshot }: { snapshot: VaultSnapshot }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex-1 overscroll-contain overflow-y-auto p-3 [scrollbar-gutter:stable]">
        <div className="rounded-[0.875rem] border border-line bg-surface p-3">
          <div className="flex items-center gap-2 type-caption text-subtle">
          ...
          <div className="mt-1 flex items-center justify-between type-caption text-subtle">
            <span>{formatToken(snapshot.userShareBalance)} rtUSD</span>
            <span>1 rtUSD = ${snapshot.sharePrice.toFixed(4)}</span>
          </div>
          {!account.connected ? (
            <p className="mt-3 rounded-[0.625rem] bg-soft p-2 type-caption text-subtle">
              Connect a wallet to reveal your live position and transact.
            </p>
          ) : null}
        </div>

        <div className="mt-4 px-2 type-caption font-semibold uppercase tracking-[0.08em] text-subtle">
          Available
        </div>
        <div className="mt-1 grid gap-1">
          <div className="flex h-9 items-center gap-2 rounded-[0.5rem] px-2 type-ui text-muted">
        ...
        {faucetAvailable ? (
          <div className="mt-4 rounded-[0.875rem] border border-line bg-surface p-3">
            <p className="type-ui font-medium text-ink">
            ...
            {!account.connected ? (
              <p className="mt-2 text-center type-caption text-subtle">
                Connect a wallet first.
              </p>
            ) : null}
            {snapshot.mode === "demo" ? (
              <button
                className="mt-2 w-full text-center type-caption text-subtle hover:text-ink"
                onClick={demo.reset}
                type="button"
              >
                Reset demo state
        ...
        <div className="mt-1 grid gap-1">
          <Link
            className="flex h-9 items-center gap-2 rounded-[0.5rem] px-2 type-ui text-muted hover:bg-soft hover:text-ink"
            href="/decisions"
          >
