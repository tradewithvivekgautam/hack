...
export function Field({ label, error, description, children, className }: { label: ReactNode; error?: ReactNode; description?: ReactNode; children: ReactNode; className?: string }) {
  return <Primitive.Root invalid={Boolean(error)} className={cn("grid gap-1.5", className)}>
    <Primitive.Label className="text-xs font-medium text-muted">{label}</Primitive.Label>
    {children}
    {error ? <Primitive.Error className="text-xs text-danger">{error}</Primitive.Error> : null}
    {description ? <Primitive.Description className="text-xs leading-4 text-subtle">{description}</Primitive.Description> : null}
  </Primitive.Root>;
}
